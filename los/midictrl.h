...
class MidiCtrlValList : public std::multimap<int, MidiCtrlVal, std::less<int> >
{
    ...
    iMidiCtrlVal find(const int&)
    {
        return end();
    };

    ciMidiCtrlVal find(const int&) const
    {
        return end();
    };
...
public:
    ...
    int lastValidHWVal() const
    {
        return _lastValidHWVal;
    }
};
...
public:
    ...
    void add(MidiController* mc)
    {
        insert(std::pair<int, MidiController*>(mc->num(), mc));
    }
};
