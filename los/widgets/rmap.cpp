...
{
    setupUi(this);
    routeList->installEventFilter(los);
    ...
    if (!rows.isEmpty())
    {
        ...
        if(item)
        {
            ...
            if(f.exists() && f.isFile() && f.isWritable())
            {
                ...
                if(QMessageBox::question(this, QString("LOS: Delete?"),
...
void RouteMapDock::btnEditClicked(bool)/*{{{*/
{
    ...
    if (!rows.isEmpty())
    {
        ...
        QString note = los->noteForRouteMapping(routePath + "/" + item->text() + ".orm");
        if(item)
        {
...
void RouteMapDock::saveRouteMap(QString _name, QString note)/*{{{*/
{
    if(_name.isEmpty())
        return;
    los->saveRouteMapping(routePath + "/" + _name + ".orm", note);
    populateTable(-1);
}/*}}}*/
...
void RouteMapDock::updateRouteMap(QString _name, QString note)/*{{{*/
{
    if(_name.isEmpty())
        return;
    los->updateRouteMapping(routePath + "/" + _name + ".orm", note);
    populateTable(-1);
}/*}}}*/
...
void RouteMapDock::renameRouteMap(QStandardItem* changed)/*{{{*/
{
    ...
    if(item)
    {
        ...
        if(f.exists() && f.isFile() && f.isWritable())
        {
            ...
            {
                QMessageBox::critical(this, QString("LOS: Error"),
...
void RouteMapDock::populateTable(int /*flag*/)/*{{{*/
{
    ...
    for(int it = 0; it < files.size(); ++it)
    {
        ...
        QString note = los->noteForRouteMapping(f.filePath());
