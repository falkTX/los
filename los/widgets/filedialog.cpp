    ...
    if (!dir.exists())
    {
        ...
        if (createDir(path))
        {
            QMessageBox::critical(parent,
                    QWidget::tr("LOS: create directory"),
...
void MFileDialog::globalToggled(bool flag)
{
    if (flag)
    {
        ...
        if (lastGlobalDir.isEmpty())
            lastGlobalDir = losGlobalShare + QString("/") + baseDir; // Initialize if first time
...
void MFileDialog::userToggled(bool flag)
{
    if (flag)
    {
        ...
        {
            lastUserDir = losUser + QString("/") + baseDir; // Initialize if first time
        }

        if (testDirCreate(this, lastUserDir))
            setDirectory(losUser);
        else
...
void MFileDialog::projectToggled(bool flag)
{
    if (flag)
    {
        ...
        {
            // if project path is uninitialized, meaning it is still set to losProjectInitPath.
            // then project path is set to current pwd instead.
            //s = QString(getcwd(0,0)) + QString("/");
            s = config.projectBaseFolder;
        }
        else
            s = losProject + QString("/"); // + baseDir;

        if (testDirCreate(this, s))
            setDirectory(losProject);
        else
            setDirectory(s);
        ...
        bool overwriteWarning)
{
    ...
    if (fp == 0 && !noError)
    {
        ...
        QMessageBox::critical(parent, QWidget::tr("LOS: Open File"), s);
        return 0;
    }
    return fp;
}
