...
void TempoCanvas::mouseMoveEvent(QMouseEvent* event)/*{{{*/
{
    ...
    if (x < 0)
        x = 0;
    emit timeChanged(los->rasterVal(x));
}/*}}}*/
...
void TempoCanvas::deleteVal(int x1, int x2)
{
    ...
    if (deleteVal1(los->rasterVal1(x1), x2))
        redraw();
}
...
{
    int xx1 = los->rasterVal1(x1);
    int xx2 = los->rasterVal2(x2);
...
void TempoCanvas::newValRamp(int x1, int y1, int x2, int y2)/*{{{*/
{
    ...
    //xx1 = los->rasterVal1(x1);
    //xx2 = los->rasterVal2(x2);
