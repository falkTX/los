...
bool initMidiAlsa()
{
    ...
    int port = snd_seq_create_simple_port(alsaSeq, "LOS Port 0",
    ...
    losPort.port = port;
    losPort.client = snd_seq_client_id(alsaSeq);
...
void alsaScanMidiPorts()
{
    ...
    while (snd_seq_query_next_client(alsaSeq, cinfo) >= 0)
    {
        ...
        while (snd_seq_query_next_port(alsaSeq, pinfo) >= 0)
        {
            ...
            if (adr.client == losPort.client && adr.port == losPort.port)
                continue;
            int flags = 0;
            if (capability & outCap)
                flags |= 1;
    ...
    for (;;)
    {
        ...
        switch (ev->type)
        {
            ...
            case SND_SEQ_EVENT_SYSEX:
                ...
                {
                    printf("LOS: alsaProcessMidiInput sysex chunks not supported!\n");
                    break;
                }
